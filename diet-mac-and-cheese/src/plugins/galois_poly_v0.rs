use super::Plugin;
use crate::circuit_ir::{GatesBody, TypeId, TypeSpecification, TypeStore, WireCount, GateM};
...
impl Plugin for GaloisPolyV0 {
    ...
    fn gates_body(
        ...
        if operation == "prod_eq" {
            Self::prod_eq_body(params, count, output_counts, input_counts, type_store)
        } else if operation == "shift_eq" {
            Self::shift_eq_body(params, count, output_counts, input_counts, type_store)
        } else {
...
impl GaloisPolyV0 {
    ...
    fn prod_eq_body(
        params: &[String],
        count: u64, // note: count is the 
        output_counts: &[(TypeId, WireCount)], // note: not any::TypeId, use TypeStore to find that one
        input_counts: &[(TypeId, WireCount)], // note: WireCount is used to state how many of that type are present. These are then used to index with offsets from count
        type_store: &TypeStore,
    ) -> Result<GatesBody> {
        ensure!(
            params.len() == 0,
        ...
        if field == std::any::TypeId::of::<F61p>() {
            ...
            let p1_eval_wire = {
                ...
                gates.push(GateM::Mul(type_id, next_free_wire, p1_start, challenge_wire));
                let p1_eval_wire = next_free_wire + 1;
                gates.push(GateM::Add(type_id, p1_eval_wire, p1_start + 1, next_free_wire));
                p1_eval_wire
            };
            let q_eval_wire = {
            ...
            gates.push(GateM::Mul(type_id, next_free_wire, p0_eval_wire, p1_eval_wire));
            // subtract the result from q
            gates.push(GateM::Sub(type_id, next_free_wire + 1, next_free_wire, q_eval_wire));
