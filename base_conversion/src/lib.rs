...
mod tests {
    ...
    #[test]
    fn test_lookup() {
        if lookup_defined_for_mod(3,0) {
            assert_eq!(lookup_digits_mod(0,3,0), &[ 0,0,0,0,0,0 ]);
            assert_eq!(lookup_digits_mod(2,3,6).to_vec(), vec![ 2,0,1,1,2,0,1,1,2,2,0,2,1,1,0,0,2,0,1,1,1,0,2,0,1,1,2,1,0,2,2,0,0,0,0,0 ]);
        }
    }
}
