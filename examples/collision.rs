    ...
    where F: Fancy<Item=W>, W: Clone + Default + HasModulus
{
    ...
    let collision = f.or_many(&collisions);
    ...
    if check_for_cheaters {
        ...
        let delta = f.constant_bundle_binary(&util::u128_to_bits(10, nbits));

        let possible_cheats = (1..time_slices).flat_map(|t| {
            (0..3).flat_map(|d| {
                (0..4).map(|i| {
                    // ensure the difference between t and the previous t is at most delta
                    let (diff, _) = f.binary_subtraction(&inputs[t][d][i], &inputs[t-1][d][i]);
                    let abs  = f.abs(&diff);
                    f.exact_geq(&abs, &delta)
                }).collect_vec()
            }).collect_vec()
        }).collect_vec();

        let cheater_detected = f.or_many(&possible_cheats);
        let output = f.or(&collision, &cheater_detected);
