...
impl Circuit {
    ...
    pub fn read_bristol_fashion(filename: &Path, trace: Option<&DebugInfo>) -> Result<Self, Error> {
        ...
        line_to_nums(&mut reader, &mut line, &mut nums)?;
        if nums.is_empty() {
            return fail(3, "Unable to parse line");
        }
        let nov = nums[0];
        if nums.len() - 1 != nov {
        ...
        let mut wl: Vec<(usize, WireKind)> = vec![(0, WireKind::Unassigned); nwires + nconstants];
        ...
        let mut gl: Vec<(GateKind, [usize; 3])> = vec![(GateKind::Unassigned, [0; 3]); 1 + ngates]; // ignore zeroth element
        ...
        let mut map_wires_and_gates = |gwdef: &mut Vec<usize>, gate: GateKind, gatenum: usize| {
        ...
        let mut ops: Vec<Gate> = Vec::with_capacity(ngates);
        let mut owl: Vec<usize> = vec![0; tnow + nconstants];
        let mut cow = tniw;
        // Emit `nconstants` constants (see above).
        ops.push(Gate::Constant(true));
        cow += 1;
        assert_eq!(constant_wires_base + nconstants, cow);
        for i in 1..=ngates {
            let wire1 = gl[i].1[0];
            ...
            match gl[i].0 {
                GateKind::And => {
                    ...
                    ops.push(Gate::And(wl[wire1].0, wl[wire2].0));
                ...
                GateKind::Xor => {
                    ...
                    assert!(wire2 < tniw && wk2 == WireKind::Input || wk2 == WireKind::Output);
                    assert!(wk3 == WireKind::Output);
                    ops.push(Gate::Xor(wl[wire1].0, wl[wire2].0));
                    if wire3 >= nwires - tnow {
                        owl[tnow - (nwires - wire3)] = cow;
                    };
                }
                ...
                GateKind::Inv => {
                    ...
                    assert!(wire1 < tniw && wk1 == WireKind::Input || wk1 == WireKind::Output);
                    assert!(wk2 == WireKind::Output);
                    ops.push(Gate::Xor(wl[wire1].0, constant_one));
                    if wire2 >= nwires - tnow {
                        owl[tnow - (nwires - wire2)] = cow;
                    };
                }
        ...
        for i in 0..tnow {
            ...
            ops.push(Gate::EqW(owl[i + nconstants]));
        }

        Ok(Circuit { num_inputs: tniw, num_outputs: tnow, gates: ops })
    }
}
