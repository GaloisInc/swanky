...
use crate::{
    ...
    pprf::{Fp, Fp2, Fpstar, PprfReceiver, PprfSender, PPRF as PPRFTrait},
};
use rand::{CryptoRng, Rng, RngCore, SeedableRng};

use crate::{
    pprf::{
        pprf::{read_fp, write_fp},
        Tpprfreceiver,
        Tpprfsender,
        PPRF,
    },
    vole::{Rvolereceiver, Rvolesender},
};
use scuttlebutt::{AbstractChannel, AesRng, Block, Block512, Channel, Malicious};
...
impl<RVOLE: Rvolesender, PS: PprfSender, PT: PPRFTrait> Tpprfsender for Sender<RVOLE, PS, PT> {
    ...
    fn send<C: AbstractChannel>(&mut self, channel: &mut C, x: Fp) -> Result<(), Error> {
        ...
        let tau_vec: Vec<Fp> = (0..Params::N + 2)
            .map(|_| read_fp(channel).unwrap())
            .collect();
        let mut _tau: Fp = tau_vec[0];
        _tau.mul_assign(&x);
        _tau.add_assign(&chi);
        ...
        for j in 0..Params::T {
            let mut _temp = tau_vec[0];
            let v: Fp = (0..Params::N).fold(Field::zero(), |mut sum, i| {
...
impl<RVOLE: Rvolereceiver, PR: PprfReceiver, PT: PPRFTrait> Tpprfreceiver
    for Receiver<RVOLE, PR, PT>
{
    fn init() -> Result<Self, Error> {
        Ok(Self {
            _rv: PhantomData::<RVOLE>,
    ...
    fn receive<C: AbstractChannel>(
        ...
        let ots: Vec<Option<(Vec<Block>, (Fp, Fp))>> = (0..Params::T)
            .map(|i| receiver.receive(channel, rblocks[i]))
            .collect();
        ...
        if ots.iter().any(|x| *x == None) {
            ...
            /// Receiver checks
            let checks: Vec<bool> = (0..Params::T)
                .map(|j| {
