...
impl<FE: FiniteField + HasModulus2> SenderConv<FE> {
    ...
    fn bit_add_carry<C: AbstractChannel, RNG: CryptoRng + Rng>(
        ...
        for i in 0..m {
            ...
            let and_res_mac_batch = self.fcom_f2.f_input_batch_pregenerated(
                channel,
                &and_res_batch,
                &mult_input_mac[i * x_batch_len..(i + 1) * x_batch_len],
            )?;
...
mod tests {
    ...
    fn test_bit_add_carry<FE: FiniteField + HasModulus2>() -> () {
        ...
        let handle = std::thread::spawn(move || {
            ...
            let mult_input_mac = fconv
                .fcom_f2
                .f_random_batch(&mut channel, &mut rng, power)
                .unwrap();
            let (res, c) = fconv
                .bit_add_carry(
                    &mut channel,
